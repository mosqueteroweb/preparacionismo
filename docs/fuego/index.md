# 🔥 Fuego

Prevención, evacuación y extinción de incendios:

<div class="grid cards" markdown>

- __Cómo apagar un fuego de cocina__

    ---

    Vídeos prácticos sobre cómo actuar ante un incendio en la cocina de forma segura.

    [:octicons-arrow-right-24: Leer](como-apagar-un-fuego-de-cocina.md)

- __Cómo usar un extintor__

    ---

    Vídeo demostrativo sobre el uso correcto de un extintor de incendios.

    [:octicons-arrow-right-24: Leer](como-usar-un-extintor.md)

- __Decálogo: qué hacer si te evacuan de casa por un incendio cercano__

    ---

    Decálogo de evacuación ante un incendio cercano.

    [:octicons-arrow-right-24: Leer](decalogo-que-hacer-si-te-evacuan-de-casa-por-un-incendio-cercano.md)

- __Supervivencia: agua, fuego, cocina y herramientas improvisadas__

    ---

    Técnicas de supervivencia con recursos del entorno, con resumen.

    [:octicons-arrow-right-24: Leer](supervivencia-agua-fuego-cocina-y-herramientas-improvisadas.md)

</div>
